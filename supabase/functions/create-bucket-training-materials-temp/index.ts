...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/*", "application/pdf", "video/mp4", "application/vnd.ms-powerpoint", "application/vnd.openxmlformats-officedocument.presentationml.presentation"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for training-materials" ON storage.objects FOR SELECT USING (bucket_id = 'training-materials');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for training-materials" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'training-materials');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for training-materials" ON storage.objects FOR UPDATE USING (bucket_id = 'training-materials');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for training-materials" ON storage.objects FOR DELETE USING (bucket_id = 'training-materials');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'training-materials',
        public: true,
        allowed_mime_types: ["image/*", "application/pdf", "video/mp4", "application/vnd.ms-powerpoint", "application/vnd.openxmlformats-officedocument.presentationml.presentation"],
