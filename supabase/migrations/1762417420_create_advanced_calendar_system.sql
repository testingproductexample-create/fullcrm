...
ALTER TABLE appointment_resources ADD COLUMN location_id UUID;
CREATE INDEX idx_appointment_resources_location ON appointment_resources(location_id);
...
CREATE TABLE staff_skills (
    ...
    UNIQUE(organization_id, staff_id, skill_name)
);
...
CREATE TABLE external_calendar_connections (
    ...
    UNIQUE(organization_id, user_id, provider, calendar_id)
);
...
CREATE TABLE resource_utilization_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    organization_id UUID NOT NULL,
    location_id UUID,
...
CREATE TABLE staff_utilization_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    organization_id UUID NOT NULL,
    location_id UUID,
    staff_id UUID NOT NULL,
...
CREATE OR REPLACE FUNCTION update_advanced_calendar_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER trigger_business_locations_updated_at
    ...
    FOR EACH ROW
    EXECUTE FUNCTION update_advanced_calendar_updated_at();
