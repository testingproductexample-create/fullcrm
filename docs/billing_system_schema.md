...
## Overview
Complete UAE-compliant invoice and billing system with AED currency support, 5% VAT calculations, payment tracking, and integration with existing financial and order management systems.
...
### 1. invoices
Main invoice records table for storing all invoice information.
...
CREATE TABLE invoices (
  ...
  -- Status options: draft, sent, viewed, paid, partial, overdue, cancelled, void
  ...
  -- Constraints
  CONSTRAINT valid_status CHECK (status IN ('draft', 'sent', 'viewed', 'paid', 'partial', 'overdue', 'cancelled', 'void')),
  CONSTRAINT positive_amounts CHECK (subtotal_aed >= 0 AND total_amount_aed >= 0 AND paid_amount_aed >= 0),
...
CREATE INDEX idx_invoices_balance ON invoices(balance_due_aed) WHERE balance_due_aed > 0;
...
### 2. invoice_items
Line items for each invoice (services, materials, custom charges).
...
CREATE TABLE invoice_items (
  ...
  -- Type options: service, material, labor, custom, discount
  ...
  -- Reference to source (order item, service, etc.)
...
CREATE INDEX idx_invoice_items_order ON invoice_items(invoice_id, display_order);
...
### 3. payments
Payment records for invoice payments and tracking.
...
CREATE TABLE payments (
  ...
  -- Methods: cash, card, bank_transfer, check, online, wallet
...
CREATE TABLE invoice_templates (
  ...
  -- Content includes: header, footer, colors, fonts, layout settings
...
CREATE INDEX idx_invoice_templates_active ON invoice_templates(organization_id, is_active) WHERE is_active = true;
...
CREATE TABLE invoice_settings (
  ...
  -- TRN (Tax Registration Number) for UAE
...
-- Indexes
CREATE UNIQUE INDEX idx_invoice_settings_organization ON invoice_settings(organization_id);
...
CREATE TABLE invoice_history (
  ...
  -- Actions: created, updated, sent, viewed, paid, partial_payment, overdue, cancelled, voided, reminded
...
-- Indexes
CREATE INDEX idx_invoice_history_invoice ON invoice_history(invoice_id, change_date DESC);
CREATE INDEX idx_invoice_history_action ON invoice_history(action_type);
CREATE INDEX idx_invoice_history_date ON invoice_history(change_date DESC);
CREATE INDEX idx_invoice_history_user ON invoice_history(changed_by);
...
- When invoice is paid → Create revenue_tracking record
- `revenue_tracking.reference_type = 'invoice'`
- `revenue_tracking.reference_id = invoice.id`

**User Integration**:
- `invoices.created_by` → `auth.users.id`
- `payments.processed_by` → `auth.users.id`
- Track who creates/modifies invoices
...
```
Subtotal = Sum of (invoice_items.total_aed)
Discount = invoice.discount_amount_aed
...
1. Add payment record to `payments` table
2. Update `invoices.paid_amount_aed`
3. Recalculate `invoices.balance_due_aed`
4. Update `invoices.status` based on balance
5. Create history record
6. Create revenue_tracking record
...
**invoices**: 8 indexes
- organization_id, customer_id, order_id, invoice_number
- status, issue_date, due_date, balance_due

**invoice_items**: 4 indexes
- invoice_id, item_type, reference, display_order

**payments**: 6 indexes
- organization_id, invoice_id, payment_date
- status, method, transaction_reference

**invoice_templates**: 3 indexes
- organization_id, is_default, is_active

**invoice_settings**: 1 index
- organization_id (unique)

**invoice_history**: 4 indexes
- invoice_id+date, action_type, change_date, changed_by
...
### Invoices
- 10 invoices across different statuses
- Mix of paid, partial, pending, overdue
- Different customers and order references
- Various amounts (AED 500 - AED 5,000)

### Invoice Items
- Services: Tailoring, alterations, custom designs
- Materials: Fabrics, buttons, threads
- Labor charges
- 3-5 items per invoice
...
- UAE-compliant invoice template
- Bank details and TRN
...
- VAT rate default: 5% (UAE standard)
- Invoice numbers auto-increment with prefix
- Full audit trail via invoice_history
- RLS enabled on all tables
- Multi-tenant using organization_id
- Soft delete support via status changes
